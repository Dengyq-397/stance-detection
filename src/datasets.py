...
class PStance(Dataset):
    def __init__(self, phase, topic, model='bert-base', wiki_model=''):
        ...
        if wiki_model:
            ...
            if topic=='biden' or topic=='trump':
                t = {'biden': 'joe biden', 'bernie': 'bernie sanders', 'trump': 'donald trump'}[topic]
                wiki_summary = wiki_dict[t]
            elif topic=="leni":
                file = codecs.open("./data/pstance/Leni_Robredo.txt",'r',encoding='utf-8')
                wiki_summary=file.read()
            elif topic=="bongbong":
                file = codecs.open("./data/pstance/Bongbong_Marcos.txt",'r',encoding='utf-8')
                wiki_summary=file.read()
            #print("wiki_dicts"+wiki_summary)
            if wiki_model == model:
